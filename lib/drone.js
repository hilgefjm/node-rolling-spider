    ...
    switch (data[4]) {
      case CLAW_STATE:
        ...
        if (this.clawState === this.CLAW_STATES.CLAW_STATE_OPENED) {
          this.emit('claw-opened');
        } else if (this.clawState === this.CLAW_STATES.CLAW_STATE_CLOSED) {
          this.emit('claw-closed');
        }
        break;
      case GUN_STATE:
...
function fire(callback) {
  if (this.gunState === this.GUN_STATES.GUN_STATE_READY) {
    ...
    this.writeTo('fa0a', Buffer.from([2, ++this.steps.fa0a, 2, 16, 2, 0, 0, 0, 0, 0, 0]));
    if (callback && typeof callback === 'function') {
      this.once('gun-ready', callback);
    }
  }
}
 ...
 */
function closeClaw(callback) {
  if (this.clawState === this.CLAW_STATES.CLAW_STATE_OPENED) {
    this.logger('RollingSpider#closeClaw');
    this.writeTo('fa0a', Buffer.from([2, ++this.steps.fa0a, 2, 16, 2, 0, 0, 1, 0, 0, 0]));
    if (callback && typeof callback === 'function') {
      this.once('claw-closed', callback);
    }
  }
}
 ...
 */
function toggleClaw(callback) {
  this.clawState ? this.openClaw(callback) : this.closeClaw(callback);
}
