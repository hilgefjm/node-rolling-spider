...
Drone.prototype.connect = function(callback) {
  ...
  this.ble.on('discover', function(peripheral) {
    ...
    if (!this.discovered) {
      console.log('Device name: \'' + localName + '\', UUID: \'' + uuid + '\'');
...
Drone.prototype.handshake = function(callback) {
  ...
  this.getCharacteristic('fb0e').on('data', function(data, isNotification) {
    ...
    if (handleUsbAccessoryData.call(this, data)) {
      return;
    }
...
var MAMBO_ID = 2;
var USB_ACCESSORY_STATE = 15;
var CLAW_STATE = 1;
var GUN_STATE = 2;
var USB_ACCESSORY = 16;
var GUN_CONTROL = 2;
var GUN_FIRE = 0;
var CLAW_CONTROL = 2;
var CLAW_OPEN = 0;
...
function handleUsbAccessoryData(data) {
  console.log(data);
  if (data[3] === USB_ACCESSORY_STATE) {
    switch (data[4]) {
      case CLAW_STATE:
      console.log('CLAW');
      this.clawState = data[7];
      break;
      case GUN_STATE:
      console.log('GUN');
      this.isFiring = data[7] ? true : false;
      break;
    }
    return true;
  }
...
/*
http://forum.developer.parrot.com/t/minidrone-characteristics-uuid/4686
http://forum.developer.parrot.com/t/clawcontrol-and-guncontrol/6365
*/
// Set up gun state
Drone.prototype.gunState = 0;
Drone.prototype.GUN_STATES = {
  GUN_STATE_READY: 0,
  GUN_STATE_BUSY: 1
};

/**
 * Fires the gun on the minidrone
 */
function fire() {
  if (this.gunState === this.GUN_STATES.GUN_STATE_READY) {
    this.logger('RollingSpider#fire');

    var buffer = new Buffer(19);
    buffer.fill(0);
    buffer.writeInt16LE(2, 0); // NO_ACK
    buffer.writeInt16LE(0, 1); // COUNTER
    buffer.writeInt16LE(MAMBO_ID, 2); // PROJECT = minidrone
    buffer.writeInt16LE(USB_ACCESSORY, 3); // CLASS = USB_ACCESSORY
    buffer.writeInt16LE(GUN_CONTROL, 4); // CMD
    buffer.writeInt16LE(0, 5); // 0
    buffer.writeInt16LE(0, 6); // usb_accessory_id
    buffer.writeInt16LE(GUN_FIRE, 7); // enum
    // buffer.writeInt16LE(0, 8);
    // buffer.writeInt16LE(0, 9);
    // buffer.writeInt16LE(0, 10);
    // buffer.writeFloatLE(2, 11);
    this.writeTo('fa0a', buffer);
  }
}

// Set up claw state
Drone.prototype.clawState = 0;
Drone.prototype.CLAW_STATES = {
  CLAW_STATE_OPENED: 0,
  CLAW_STATE_OPENING: 1,
  CLAW_STATE_CLOSED: 2,
  CLAW_STATE_CLOSING: 3
};

/**
 * Commands the claw to open
 */
function openClaw() {
  if (this.clawState === this.CLAW_STATES.CLAW_STATE_CLOSED) {
    this.logger('RollingSpider#openClaw');
    var buffer = new Buffer(19);
    buffer.fill(0);
    buffer.writeInt16LE(2, 0); // NO_ACK
    buffer.writeInt16LE(0, 1); // Not sure what this byte is
    buffer.writeInt16LE(MAMBO_ID, 2); // PROJECT = minidrone
    buffer.writeInt16LE(USB_ACCESSORY, 3); // CLASS = USB_ACCESSORY
    buffer.writeInt16LE(CLAW_CONTROL, 4); // CMD
    buffer.writeInt16LE(0, 5); // 0
    buffer.writeInt16LE(0, 6); // usb_accessory_id
    buffer.writeInt16LE(CLAW_OPEN, 7); // enum
    this.writeTo('fa0a', buffer);
  }
}

/**
 * Commands the claw to close
 */
function closeClaw() {
  if (this.clawState === this.CLAW_STATES.CLAW_STATE_OPENED) {
    this.logger('RollingSpider#closeClaw');
    var buffer = new Buffer(19);
    buffer.fill(0);
    buffer.writeInt16LE(4, 0); // WITH_ACK
    buffer.writeInt16LE(3, 1); // Not sure what this byte is
    buffer.writeInt16LE(MAMBO_ID, 2); // PROJECT = minidrone
    buffer.writeInt16LE(USB_ACCESSORY, 3); // CLASS = USB_ACCESSORY
    buffer.writeInt16LE(CLAW_CONTROL, 4); // CMD
    buffer.writeInt16LE(0, 5); // 0
    buffer.writeInt16LE(0, 6); // usb_accessory_id
    buffer.writeInt16LE(CLAW_CLOSE, 7); // enum
    this.writeTo('fa0b', buffer);
  }
}

/**
 * Commands the claw to open if closed, or close if opened
 */
function toggleClaw() {
  this.clawState ? this.openClaw() : this.closeClaw();
}
...
// USB Accessories
Drone.prototype.openClaw = openClaw;
Drone.prototype.closeClaw = closeClaw;
Drone.prototype.toggleClaw = toggleClaw;
Drone.prototype.fire = fire;
